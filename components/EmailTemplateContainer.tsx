...
const EmailTemplateContainer = ({ data }: { data: any }) => {
  ...
  const template = [...templateData].find((e: any) =>
    e.type.includes(params.template)
  );
  const [emailBody, setEmailBody] = useState({
    body:
      (template && `${parser(template?.body)}`.trim() !== ""
        ? template?.body
        : "<p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Non totam blanditiis voluptatem aliquid ducimus esse vitae impedit explicabo! Accusantium molestias, officia, exercitationem nemo quas deserunt ex natus quam iusto quo recusandae fugit.</p>") ||
      "",
