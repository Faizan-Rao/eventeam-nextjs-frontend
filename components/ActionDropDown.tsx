  ...
  return (
    <>
      <DropdownMenu modal={false}>
        <DropdownMenuTrigger
          className={clsx(
            "active:scale-[0.90] rounded-full   p-1 hover:bg-[#7655fa26] transition-all"
          )}
        >
          <EllipsisVertical
            className={clsx(selectedRecord === id && "text-[white]")}
            size={20}
          />
        </DropdownMenuTrigger>
        <DropdownMenuContent>
          <DropdownMenuItem
            className="text-sm active:scale-[0.95] transition-all"
            onClick={() => setEditOpen(true)}
          >
            {t("Edit Status")}
          </DropdownMenuItem>
          <Link href={`/dashboard/my-events/${row.id}`}>
            {" "}
            <DropdownMenuItem className="text-sm active:scale-[0.95] transition-all">
              {t("View Event")}
            </DropdownMenuItem>
          </Link>
          <a href={`/dashboard/my-events/edit/${row.id}`}>
            {" "}
            <DropdownMenuItem className="text-sm active:scale-[0.95] transition-all">
              {t("Edit Event")}
            </DropdownMenuItem>
          </a>
          <a href={`/events/${user.slug}/${row.id}`}>
            <DropdownMenuItem className="text-sm active:scale-[0.95] transition-all">
