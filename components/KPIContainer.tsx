...
const KPIContainer = () => {
  ...
  return (
    ...
    <div className="sm:hidden md:flex  items-center sm:justify-center lg:justify-center gap-4 flex-wrap">
      ...
      <KPICard title={t("All Time Events")} icon={<Calendar  size={28}/>} value={kpis?.data.data["total_events"] || "0"}/>
      <KPICard title={t("Active Events")} icon={<Calendar  size={28}/>} value={ kpis?.data.data["active_events"] || "0"} />
      <KPICard title={t("Automatic Events")} icon={<CircuitBoard  size={28}/>} value={kpis?.data.data["automatic_events"] || "0"}/>
      <KPICard title={t("Total Registrations")} icon={<UserPlus  size={28}/>} value={kpis?.data.data["total_registrations"] || "0"} />
      <KPICard title={t("All Time Guests")} icon={<UsersRound size={28}/>} value={kpis?.data.data["total_guests"] || "0"} />
      <KPICard title={t("All Time Earnings")} icon={<CircleDollarSign size={28}/>} value={kpis?.data.data["total_earnings"] || "0"} currency="$" />
      <KPICard title={t("Cleared Earnings")} icon={<HandCoins size={28}/>} value={kpis?.data.data["cleared_cash"] || "0"} currency="$"/>
      <KPICard title={t("Pending Earnings")} icon={<Banknote size={28}/>} value={kpis?.data.data["pending_cash"] || "0"} currency="$"/>
    </div>
